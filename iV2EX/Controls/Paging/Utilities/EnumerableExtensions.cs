...
namespace MyToolkit.Utilities
{
    ...
    {
        /// <summary>
        ///     Provides ordering by two expressions. Use this method instaed of OrderBy(...).ThenBy(...) as it calls ThenBy
        ///     only if necessary.
        /// </summary>
        public static IEnumerable<TSource> OrderByThenBy<TSource, TKey1, TKey2>(this IEnumerable<TSource> source,
            Func<TSource, TKey1> orderBy, Func<TSource, TKey2> thenBy)
        {
            var sorted = source
        ...
        public static bool IsCopyOf<T>(this IList<T> list1, IList<T> list2)
        {
            ...
            // Has same order
            if (list1.Where((t, i) => !Equals(t, list2[i])).Any())
                return false;
        ...
        public static IEnumerable<T> Shuffle<T>(this IEnumerable<T> source)
        {
            ...
            return source.Select(t => new KeyValuePair<int, T>(rand.Next(), t)).OrderBy(pair => pair.Key)
        ...
        {
            var resultValue = default(TProperty);
            T result = null;
            foreach (var t in list)
            {
                var value = selector(t);
