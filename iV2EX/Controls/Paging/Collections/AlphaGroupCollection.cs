    ...
    public class AlphaGroupCollection<T> : List<Group<T>>, INotifyCollectionChanged
    {
        ...
        public void Initialize(IEnumerable<T> items)
        {
            ...
            var itemGroups = items
                .OrderBy(i => i.ToString()).GroupBy(i => GetFirstCharacter(i.ToString())).ToDictionary(g => g.Key);
        ...
        private void RaiseGroupChanged(Group<T> group)
        {
            ...
            CollectionChanged?.Invoke(this,
                new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Replace, group, group, index));
        }
        ...
        private Group<T> AddItemToCorrectGroup(T item, bool searchPosition)
        {
            ...
            if (searchPosition && group.Count > 0)
            {
                ...
                var newIndex = group
                    .TakeWhile(i => string.Compare(i.ToString(), newTitle, StringComparison.Ordinal) <= 0).Count();
