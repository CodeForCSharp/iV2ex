...
{
    /// <summary>
    ///     Implementation of the <see cref="INotifyPropertyChanged" /> interface with <see cref="INotifyDataErrorInfo" />
    ///     support.
    /// </summary>
    public class ValidatedObservableObject : ObservableObject, INotifyDataErrorInfo
    {
        ...
        public IEnumerable<string> InvariantErrors => GetErrors(string.Empty);
        ...
        protected HashSet<string> NotValidatedProperties { get; }
        ...
        public void Validate()
        {
            foreach (var property in GetType().GetRuntimeProperties()
                .Where(p => p.CanRead && !NotValidatedProperties.Contains(p.Name)))
            {
        ...
        static ValidatedObservableObject()
        {
            try
            {
                ...
                ValidationResultsConstructor = validationResultsType.GetTypeInfo().DeclaredConstructors
                    .Single(m => m.IsConstructor && m.GetParameters().Length == 0);
                ValidationContextConstructor = validationContextType.GetTypeInfo().DeclaredConstructors
                    .Single(m => m.IsConstructor && m.GetParameters().Length == 1);
            }
            catch (Exception exception)
            {
                throw new NotSupportedException(
                    "The ValidatedObservableObject class cannot be used with the current .NET framework. ", exception);
            }
        }
        ...
        private static bool InvokeTryValidatePropertyMethod(object value, dynamic validationContext,
            IEnumerable validationResults)
        {
            return (bool) TryValidatePropertyMethod.Invoke(null,
                new object[] {value, validationContext, validationResults});
        }
