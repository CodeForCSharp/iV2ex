...
namespace MyToolkit.Paging
{
    ...
    public class MtPageDescription
    {
        ...
        public bool IsInstantiated => Page != null;

        /// <summary>Gets the page type. </summary>
        public Type Type => _type ?? (_type = Type.GetType(SerializationType));
        ...
        [DataMember] internal string PageKey { get; set; }

        [DataMember] internal string SerializationType { get; set; }
        ...
        internal MtPage GetPage(MtFrame frame)
        {
            if (Page == null)
            {
                if (!(Activator.CreateInstance(Type) is MtPage page))
                    throw new InvalidOperationException(
                        "The base type is not an MtPage. Change the base type from Page to MtPage. ");
