...
namespace MyToolkit.UI
{
    public static class PopupHelper
    {
        ...
        /// </summary>
        public static Task<Popup> ShowHorizontalDialogAsync(FrameworkElement control,
            bool isLightDismissEnabled = false)
        {
            return TaskUtilities.RunCallbackMethodAsync<FrameworkElement, Popup>(
                (x, y) => ShowDialog(x, isLightDismissEnabled, true, y), control);
        }
        ...
        {
            return TaskUtilities.RunCallbackMethodAsync<FrameworkElement, Popup>(
                (x, y) => ShowDialog(x, isLightDismissEnabled, false, y), control);
        }
        ...
        public static Popup ShowHorizontalDialog(FrameworkElement control, bool isLightDismissEnabled = false,
            Action<Popup> closedCallback = null)
        {
        ...
        public static Popup ShowVerticalDialog(FrameworkElement control, bool isLightDismissEnabled = false,
            Action<Popup> closedCallback = null)
        {
            return ShowDialog(control, isLightDismissEnabled, false, closedCallback);
        }

        private static Popup ShowDialog(FrameworkElement control, bool isLightDismissEnabled = false,
            bool isHorizontal = true, Action<Popup> closedCallback = null)
        {
            var popup = new Popup();
            ...
            {
                if (((Frame) parent).Content is Page page)
                {
                    if (page.TopAppBar != null)
                    {
                        topAppBarVisibility = page.TopAppBar.Visibility;
        ...
        {
            if (isHorizontal)
                popup.VerticalOffset = Window.Current.Bounds.Top +
                                       (Window.Current.Bounds.Height - control.ActualHeight) / 2;
            else
                popup.HorizontalOffset = Window.Current.Bounds.Left +
                                         (Window.Current.Bounds.Width - control.ActualWidth) / 2;
        }
