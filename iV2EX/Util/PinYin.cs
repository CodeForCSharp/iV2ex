...
namespace iV2EX.Util
{
    ...
    public static class PinYin
    {
        ...
        public static string GetFirstPinyin(string strText)
        {
            ...
            var vChar = strText.ToCharArray()[0];

            // 若是字母则直接返回
            if (vChar >= 'a' && vChar <= 'z' || vChar >= 'A' && vChar <= 'Z')
                myStr = vChar.ToString();
            else if (vChar >= 19968 && vChar <= 40869)
                myStr = StrChineseFirstPy[vChar - 19968].ToString();
        ...
        public static string AddFirstPinyin(string str)
        {
            ...
            var vChar = str.ToCharArray()[0];
            // 若是字母则直接返回
            if (vChar >= 'a' && vChar <= 'z' || vChar >= 'A' && vChar <= 'Z')
                return str;
            if (vChar >= 19968 && vChar <= 40869)
            {
                // 对可以查找的汉字计算它的首拼音字母的位置，然后输出
                var strNew = StrChineseFirstPy[vChar - 19968].ToString();
                return strNew + str;
            }
